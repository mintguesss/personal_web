...
export default function SkillsAwardsPage() {
  ...
  const AwardCard = ({ a, delay = 0 }: { a: Award; delay?: number }) => {
    ...
    return (
      <R delay={delay}>
        <div
          ...
          style={{ border: '1px solid var(--award1)', borderRadius: '15px', background: 'var(--bg)', overflow: 'hidden', cursor: clickable ? 'pointer' : 'default', transition: 'transform 0.18s ease, border-color 0.18s ease, box-shadow 0.18s ease' }}
          onMouseEnter={e => { if (!clickable) return; const el = e.currentTarget as HTMLDivElement; el.style.borderColor = 'var(--border3)'; el.style.boxShadow = '0 8px 22px rgba(0,0,0,0.08)'; el.style.transform = 'translateY(-3px)' }}
          onMouseLeave={e => { const el = e.currentTarget as HTMLDivElement; el.style.borderColor = 'var(--award1)'; el.style.boxShadow = 'none'; el.style.transform = 'translateY(0)' }}
        >
          <div style={{ position: 'relative', width: '100%', aspectRatio: '4/3', background: 'var(--bg-2)', overflow: 'hidden' }}
          ...
          >
            {a.image
              ...
              : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontFamily: 'var(--font-display)', fontSize: '3rem', fontWeight: 300, color: 'var(--border-2)' }}>✦</span></div>
            }
            {clickable && (
              <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.38)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: imgHovered ? 1 : 0, transition: 'opacity 0.2s ease', pointerEvents: 'none' }}>
                {a.link ? <ExternalIcon size={22} stroke="#fff" /> : <ZoomIcon size={22} stroke="#fff" />}
              </div>
            )}
          </div>
          <div style={{ padding: '1.1rem 1.25rem', position: 'relative' }}>
            {clickable && <div style={{ position: 'absolute', top: '1rem', right: '1rem', opacity: 0.35 }}>{a.link ? <ExternalIcon /> : <ZoomIcon />}</div>}
            ...
            {a.link && <a href={a.link} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ display: 'none' }} />}
          </div>
        </div>
      </R>
    )
      ...
      <div style={{ borderTop: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: `0 ${PAD} clamp(3rem,5vw,5rem)` }}>
          ...
          <div style={{ padding: '2rem 0', borderBottom: '1px solid var(--border)' }}>
            <R><p style={{ ...MONO, fontSize: '0.78rem', marginBottom: '1.25rem' }}>Competitions</p></R>
            <div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : 'repeat(2,1fr)', gap: mobile ? '2rem' : '6rem' }}>
              {competition.map((a, i) => (
                <AwardCard key={`${a.year}-${a.title}`} a={a} delay={80 + i * 100} />
              ))}
            </div>
          </div>
          ...
          <div style={{ padding: '2.5rem 0' }}>
            <R><p style={{ ...MONO, fontSize: '0.78rem', marginBottom: '1.25rem' }}>在校榮譽</p></R>
            <div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr', gap: '1rem 5rem' }}>
              {school.map((a, i) => {
                const clickable = !!(a.image || a.link)
                return (
                  <R key={`${a.year}-${a.title}`} delay={60 + i * 50}>
                    <div
                      onClick={() => a.image ? setModalImg(a.image) : undefined}
                      style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start', padding: '0.9rem 0.75rem', border: '1px solid var(--bg)', borderRadius: '6px', background: 'var(--bg)', cursor: clickable ? 'pointer' : 'default', transition: 'transform 0.18s ease, border-color 0.18s ease, box-shadow 0.18s ease', position: 'relative' }}
                      onMouseEnter={e => { if (!clickable) return; const el = e.currentTarget as HTMLDivElement; el.style.borderColor = 'var(--accent)'; el.style.boxShadow = '0 8px 22px rgba(0,0,0,0.08)'; el.style.transform = 'translateY(-3px)' }}
                      onMouseLeave={e => { const el = e.currentTarget as HTMLDivElement; el.style.borderColor = 'var(--bg)'; el.style.boxShadow = 'none'; el.style.transform = 'translateY(0)' }}
                    >
                      ...
                      {clickable && <div style={{ opacity: 0.35, flexShrink: 0, marginTop: '0.2rem' }}>{a.link ? <ExternalIcon /> : <ZoomIcon />}</div>}
                    </div>
                  </R>
                )
              })}
